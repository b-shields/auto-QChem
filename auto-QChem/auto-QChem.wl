...
(***************************************************************************************************** set directories and indices*)
	Which[
		fileType=="chemdraw",
		inputPath=Directory[]<>"\\data\\chemdraw_input\\",
		fileType=="smiles",
		inputPath=Directory[]<>"\\data\\smiles_input\\"
	];

	outputPath=Directory[]<>"\\data\\coordinate_input\\"<>fileName;

(***************************************************************************************************** check to see if a file with the name already exists*)
	If[
		FileExistsQ[Directory[]<>"\\data\\coordinate_input\\"<>fileName],
		runBatchQ=False,
		runBatchQ=True
	];
...
(***************************************************************************************************** generate coordinates from ChemDraw input*)
			...
			CreateDirectory[Directory[]<>"\\data\\coordinate_input\\"<>fileName];
			(*generate open babel command*)
			babelCommand="babel -icdx "<>inputPath<>fileName<>".cdx -osmiles "<>Directory[]<>"\\data\\smiles_input\\"<>fileName<>".txt";
			(*generate smiles*)
			RunProcess[{"cmd","/c",babelCommand}];	
			(*generate open babel command*)
			babelCommand="babel "<>
				"-ismiles "<>Directory[]<>"\\data\\smiles_input\\"<>fileName<>".txt"<>
				" -ogjf "<>Directory[]<>"\\data\\coordinate_input\\"<>fileName<>"\\"<>fileName<>".txt -m -h --gen3d";
			(*generate coordinates*)
			RunProcess[{"cmd","/c",babelCommand}];
			},
...
(***************************************************************************************************** generate coordinates from smiles input*)
			...
			CreateDirectory[Directory[]<>"\\data\\coordinate_input\\"<>fileName];
			(*generate open babel command*)
			babelCommand="babel -ismiles "<>inputPath<>fileName<>".txt -ogjf "<>Directory[]<>"\\data\\coordinate_input\\"<>fileName<>"\\"<>fileName<>".txt -m -h --gen3d";
			(*generate coordinates*)
			RunProcess[{"cmd","/c",babelCommand}];
			}
		],(*Which*)
...
(***************************************************************************************************** set directories and indices*)
	outputPath=Directory[]<>"\\data\\coordinate_input\\"<>StringReplace[cdxFilePath,{Longest[__~~"\\"]->"",".cdx"->""}];

(***************************************************************************************************** check to see if a file with the name already exists*)
	If[
		FileExistsQ[Directory[]<>"\\data\\coordinate_input\\"<>StringReplace[cdxFilePath,{Longest[__~~"\\"]->"",".cdx"->""}]],
		runBatchQ=False,
		runBatchQ=True
	];
...
(***************************************************************************************************** Generate input for the first calculation*)
		...
		textJob1=StringJoin[
			...
			"%Chk=",chk1,"\n",
			"# opt=CalcAll "<>theory[[1]]<>"/"<>theory[[2]]<>" scf=qc","\n",
...
(***************************************************************************************************** Generate input for the second calculation*)
		...
		textJob2=StringJoin[
			...
			"%Oldchk=",chk1,"\n",
			"%Chk=",chk2,"\n",
			"# freq "<>theory[[1]]<>"/"<>theory[[2]]<>" scf=qc"<>" volume NMR pop=NPA density=current Geom=AllCheck Guess=Read"
		];
...
(***************************************************************************************************** Generate input for the third calculation*)
		...
		textJob3=StringJoin[
			...
			"%Oldchk=",chk2,"\n",
			"%Chk=",chk3,"\n",
			"# TD(NStates=10, Root=1) "<>theory[[1]]<>"/"<>theory[[2]]<>" scf=qc"<>" volume pop=NPA density=current Geom=AllCheck Guess=Read"
		];
		},
...
(***************************************************************************************************** Generate input for the first calculation*)
		...
		textJob1=StringJoin[Flatten[{
			...
			"%Mem=",ToString[processorsMemory[[2]]],"GB\n",
			"%Chk=",chk1,"\n",
			"# opt=CalcAll "<>theory[[1]]<>"/genecp"<>" scf=qc","\n",
			"\n","Title Card Required","\n\n",moleculeSpecs[[1]]," ",moleculeSpecs[[2]],"\n",coordinates,
...
(***************************************************************************************************** Generate input for the second calculation*)
		...
		textJob2=StringJoin[Flatten[{
			...
			"%Oldchk=",chk1,"\n",
			"%Chk=",chk2,"\n",
			"# freq "<>theory[[1]]<>"/genecp scf=qc volume NMR pop=NPA density=current Geom=AllCheck Guess=Read",
			Flatten[{"\n\n",Thread[{lightatoms,Table[" ",{n,1,Count[lightatoms,x_],1}]}]}],"0",
			"\n",theory[[2]],"\n****",
			Flatten[{"\n",Thread[{heavyatoms,Table[" ",{n,1,Count[heavyatoms,x_],1}]}]}],"0",
			"\n",theory[[3]],"\n****",
...
(***************************************************************************************************** Generate input for the third calculation*)
		...
		textJob3=StringJoin[Flatten[{
			...
			"%Oldchk=",chk2,"\n",
			"%Chk=",chk3,"\n",
			"# TD(NStates=10, Root=1) "<>theory[[1]]<>"/genecp scf=qc volume pop=NPA density=current Geom=AllCheck Guess=Read",
			Flatten[{"\n\n",Thread[{lightatoms,Table[" ",{n,1,Count[lightatoms,x_],1}]}]}],"0",
			"\n",theory[[2]],"\n****",
			Flatten[{"\n",Thread[{heavyatoms,Table[" ",{n,1,Count[heavyatoms,x_],1}]}]}],"0",
			"\n",theory[[3]],"\n****",
			...
			For[
				...
				(
					Which[
						...
						Count[StringContainsQ[rootSections,possibleSectionKeywords[[i]][[2]],IgnoreCase->True],True]==4,
						outputSections=Append[outputSections,possibleSectionKeywords[[i]][[1]]]			
					](*If*)
				)
			];
...
(***************************************************************************************************** make sure the submission directory doesn't already exist*)		
	If[
		FileExistsQ[Directory[]<>"\\data\\submission\\"<>outputDirectoryName],
...
(***************************************************************************************************** Make a working directory for submission and retreival of jobs*)
		CreateDirectory[Directory[]<>"\\data\\submission\\"<>outputDirectoryName];
	
(***************************************************************************************************** gaussian .com file from coordinates*)
		jobList=filelist[coordinateDirectoryPath];
		Table[
			Export[
				Directory[]<>"\\data\\submission\\"<>outputDirectoryName<>"\\"<>StringReplace[jobList[[i]],{x__~~"\\"->"","."~~x__->""}]<>"123.com",
				Which[
					jobType=="equilibrium",
...
(***************************************************************************************************** generate unix command files*)
		jobList=filelist[Directory[]<>"\\data\\submission\\"<>outputDirectoryName];
		jobList[[Flatten[Position[StringContainsQ[jobList,".com"],True]]]];
			
		Table[
			Export[
				Directory[]<>"\\data\\submission\\"<>outputDirectoryName<>"\\"<>StringReplace[jobList[[i]],{x__~~"\\"->"","."~~x__->""}]<>".cmd",
...
(***************************************************************************************************** generate .sh submission script*)
		...
		Export[
			Directory[]<>"\\data\\submission\\"<>outputDirectoryName<>"\\"<>coordinateDirectoryName<>".sh",
			shScriptText,
			"Text",
			DOSTextFormat->False
		];
...
(***************************************************************************************************** generate SFTP commands - just in case of manual submission*)
		Export[
			Directory[]<>"\\data\\submission\\"<>outputDirectoryName<>"\\"<>coordinateDirectoryName<>"_SFTP.txt",
			StringJoin[
				"put -r ",StringReplace[Directory[]<>"\\data\\submission\\"<>outputDirectoryName,"\\"->"/"]," ",outputDirectoryName,
				"\n\n\n",
				"get -r ",outputDirectoryName," ",StringReplace[Directory[]<>"\\data\\submission\\"<>outputDirectoryName,"\\"->"/"]
			],
			"Text"
		]
		};
...
(***************************************************************************************************** run any outstanding jobs for files in the input folders*)
	ChemDraw:={
		chemdrawJobs=StringReplace[filenames[Directory[]<>"\\data\\chemdraw_input\\"],".cdx"->""];
	...
	Smiles:={
		smilesJobs=StringReplace[filenames[Directory[]<>"\\data\\smiles_input\\"],".txt"->""];
...
(***************************************************************************************************** get list of potential jobs*)
		coordinateDirectories=filelist[Directory[]<>"\\data\\coordinate_input"];
	
(***************************************************************************************************** see if a submission file with the same batch name already exists*)
		Table[
				{
				If[
					FileExistsQ[Directory[]<>"\\data\\submission\\"<>StringReplace[coordinateDirectories[[i]],x__~~"\\"->""]],
					Null,
